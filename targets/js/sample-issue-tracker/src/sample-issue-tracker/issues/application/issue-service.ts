...
import type { IssueType } from "#/sample-issue-tracker/issues/domain/issue-type";
...
export class IssueService {
  ...
  private async createAsyncTitleDescriptionTypePriority(
    title: string,
    description: string,
    type: IssueType,
  ...
  private createAsyncTitleDescriptionType(
    title: string,
    description: string,
    type: IssueType,
  ): Promise<OperationResult<Issue>> {
  ...
  async createAsync(...args: unknown[]): Promise<unknown> {
    if (
      ...
      isString(args[0]) &&
      isString(args[1]) &&
      (args[2] === "story" || args[2] === "bug" || args[2] === "chore" || args[2] === "spike") &&
      (args[3] === "low" || args[3] === "medium" || args[3] === "high" || args[3] === "urgent")
    ) {
      return this.createAsyncTitleDescriptionTypePriority(
        args[0] as string,
        args[1] as string,
        args[2] as IssueType,
        args[3] as IssuePriority,
      );
    }
    ...
    if (
      ...
      return this.createAsyncTitleDescriptionType(
        args[0] as string,
        args[1] as string,
        args[2] as IssueType,
      );
  ...
  async loadAsync(issueId: IssueId): Promise<OperationResult<Issue>> {
    ...
    return issue == null
      ? OperationResult.fail("issue_not_found", `Issue ${issueId} was not found.`)
      : OperationResult.ok(issue);
  }
  ...
  private async addCommentAsyncIssueIdAuthorIdMessage(
    issueId: IssueId,
    authorId: UserId,
    message: string,
  ): Promise<OperationResult<Issue>> {
  ...
  private async addCommentAsyncIssueAuthorIdMessage(
    issue: Issue,
    authorId: UserId,
    message: string,
  ): Promise<OperationResult<Issue>> {
  ...
  async addCommentAsync(...args: unknown[]): Promise<unknown> {
    if (
      ...
      return this.addCommentAsyncIssueIdAuthorIdMessage(
        args[0] as IssueId,
        args[1] as UserId,
        args[2] as string,
      );
    ...
    if (
      args.length === 3 &&
      args[0] instanceof Issue &&
      typeof args[1] === "string" &&
      isString(args[2])
    ) {
      return this.addCommentAsyncIssueAuthorIdMessage(
        args[0] as Issue,
        args[1] as UserId,
        args[2] as string,
      );
