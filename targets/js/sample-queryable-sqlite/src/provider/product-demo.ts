...
export class ProductDemo {
  ...
  static activeProducts(repo: IProductRepository): Product[] {
    return repo.toArray(linq(repo.products, where((p: Product) => p.isActive, {
      tree: {
        kind: "member",
  ...
  static inStock(repo: IProductRepository): Product[] {
    return repo.toArray(linq(repo.products, where((p: Product) => p.isActive && p.stockCount > 0, {
      tree: {
        kind: "binary",
        op: "&&",
        left: {
  ...
  static atLeastPrice(repo: IProductRepository, minPrice: Decimal): Product[] {
    return repo.toArray(linq(repo.products, where((p: Product) => p.unitPrice.gte(minPrice), {
      tree: {
        kind: "binary",
        op: ">=",
        left: {
  ...
  static topByPrice(repo: IProductRepository, topN: number): Product[] {
    return repo.toArray(linq(repo.products, orderByDescending((p: Product) => p.unitPrice, {
      tree: {
        kind: "member",
        target: { kind: "param", name: "p", type: "Product" },
        member: "unitPrice",
  ...
  static page(repo: IProductRepository, offset: number, pageSize: number): Product[] {
    return repo.toArray(linq(repo.products, orderBy((p: Product) => p.id, {
      tree: {
        kind: "member",
        target: { kind: "param", name: "p", type: "Product" },
        member: "id",
  ...
  static countActive(repo: IProductRepository): number {
    return repo.count(linq(repo.products, where((p: Product) => p.isActive, {
      tree: {
        kind: "member",
        target: { kind: "param", name: "p", type: "Product" },
        member: "isActive",
  ...
  static firstById(repo: IProductRepository): Product | null {
    return repo.firstOrDefault(linq(repo.products, orderBy((p: Product) => p.id, {
      tree: {
        kind: "member",
        target: { kind: "param", name: "p", type: "Product" },
        member: "id",
