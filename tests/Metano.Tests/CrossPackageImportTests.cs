...
public class CrossPackageImportTests
{
    ...
    public async Task ConsumerReferencingLibraryType_EmitsCrossPackageImport()
    {
        ...
        var output = result["app/wallet.ts"];

        // Cross-package import is namespace-first: package name + full namespace
        // path derived from the library type's own namespace ("MyLib.Domain"), so
        // "MyLib.Domain.Money" becomes "@scope/lib/my-lib/domain".
        await Assert.That(output).Contains("import { Money } from \"@scope/lib/my-lib/domain\"");
        ...
        await Assert.That(result).DoesNotContainKey("my-lib/domain/money.ts");
    }
    ...
    public async Task SameSimpleNameInDifferentAssemblies_DisambiguatedBySymbol()
    {
        ...
        var result = TranspileHelper.TranspileWithLibrary(library, consumer);
        var output = result["app/order.ts"];

        // Cross-package import uses the library type's full namespace path ("Shared").
        await Assert.That(output).Contains("import { Status } from \"shared/shared\"");
        ...
        await Assert.That(result).ContainsKey("app/status.ts");
    }
    ...
    public async Task LibraryWithoutEmitPackage_EmitsMs0007AtConsumer()
    {
        ...
        var output = files["app/holder.ts"];
    ...
    public async Task CrossPackageNoContainerFunction_TracksDependency()
    {
        ...
        var greeter = printer.Print(files.Single(f => f.FileName == "app/greeter.ts"));

        // The flattened import line lands (precondition for the dep confirm).
        await Assert.That(greeter).Contains("from \"@acme/util/acme-util\"");
    ...
    [Test]
    public async Task EmitInFile_CrossPackageImportUsesNamespaceBarrel()
    {
        ...
        // import line with both names from the namespace barrel (not the file path).
        var library = """
            [assembly: TranspileAssembly]
        ...
        var result = TranspileHelper.TranspileWithLibrary(library, consumer);
        var output = result["app/tracker.ts"];

        // One merged import line, two names, namespace barrel path is the package
        // plus the library type's full namespace (`@acme/issues/acme-issues`), NOT
        // the per-file path (`@acme/issues/issue`).
        await Assert.That(output).Contains("from \"@acme/issues/acme-issues\"");
        await Assert.That(output).Contains("Issue");
        await Assert.That(output).Contains("IssueStatus");
        await Assert.That(output).DoesNotContain("@acme/issues/issue");
    }
    ...
    public async Task PlainObject_CrossPackage_EmitsAsTypeImport()
    {
        ...
        var result = TranspileHelper.TranspileWithLibrary(library, consumer);
        var output = result["app/handler.ts"];
        ...
        await Assert
            .That(output)
            .Contains("import type { CreateUserDto } from \"@acme/dtos/acme-dtos\"");
    ...
    public async Task CrossPackageImport_MixedValueAndType_UsesPerNameQualifier()
    {
        ...
        // entries: `import { TodoItem, type Priority } from "sample-todo"`.
        ...
        await Assert.That(output).Contains("@acme/lib/acme-lib");
    }
    ...
    public async Task PlainObject_CrossPackage_ConstructionLowersToObjectLiteral()
    {
        ...
        var result = TranspileHelper.TranspileWithLibrary(library, consumer);
        var output = result["app/factory.ts"];
    ...
    {
        // The default decimal mapping in Metano/Runtime/Decimal.cs declares
