...
public class CrossPackageImportTests
{
    ...
    public async Task ConsumerReferencingLibraryType_EmitsCrossPackageImport()
    {
        ...
        // Cross-package import is namespace-first: package name + namespace barrel
        // derived from the library's own root namespace ("MyLib"), so
        // "MyLib.Domain.Money" becomes "@scope/lib/domain".
        await Assert.That(output).Contains("import { Money } from \"@scope/lib/domain\"");
    ...
    public async Task SameSimpleNameInDifferentAssemblies_DisambiguatedBySymbol()
    {
        ...
        // Root namespace match → import directly from the package root barrel.
        await Assert.That(output).Contains("import { Status } from \"shared\"");
    ...
    public async Task EmitInFile_CrossPackageImportUsesNamespaceBarrel()
    {
        ...
        // One merged import line, two names, namespace barrel path is `@acme/issues`.
        await Assert.That(output).Contains("from \"@acme/issues\"");
    ...
    public async Task PlainObject_CrossPackage_EmitsAsTypeImport()
    {
        ...
        await Assert.That(output).Contains("import type { CreateUserDto } from \"@acme/dtos\"");
    ...
    public async Task CrossPackageImport_MixedValueAndType_UsesPerNameQualifier()
    {
        ...
        await Assert.That(output).Contains("@acme/lib");
    }
